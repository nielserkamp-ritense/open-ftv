...
See the section on authentication and authorization below.
...
PAP_PIP_STORE_RECURSE=true|false            # Flag to indicate the given folder and subfolders must be search recursively for attribute files (default false).
...
The PAP service has built-in EAM components. This may sound like a contradiction.
...
These policies are kept separate from the policies maintained by the PAP.
...
This will provide the authentication module with the users that are allowed to access the PAP service.
...
Retrieved attributes are cached in the embedded PIP.
