...
| [`code.puml`](code.puml) | [`openftv-code.png`](openftv-code.png) | Code view — the `go.work` modules: deployable apps, gateway plugins, the `eam/*` domain libraries (PEP / PDP / administration / shared base), and the foundation layer. |
...
`go.work` import graph). When you add a service to `docker/compose.yaml` or a module to
`go.work`, update the matching `.puml` and regenerate the committed PNGs:
