...
type TagPersister interface {
	...
	CreateTag(ctx context.Context, user identity.Principal, t *oas.Tag) (*oas.Tag, error)
	ReadTag(ctx context.Context, id string) (*oas.Tag, uint64, error)
	UpdateTag(ctx context.Context, user identity.Principal, prev *oas.Tag, lastIndex uint64, t *oas.Tag) (*oas.Tag, error)
	DeleteTag(ctx context.Context, user identity.Principal, prev *oas.Tag, lastIndex uint64) (*oas.Tag, error)
	EnsureTags(tags []*oas.Tag, user identity.Principal) error
}

// PolicyPersister represents the interface for managing policies in a PAP persistence store.
type PolicyPersister interface {
	CreatePolicy(ctx context.Context, user identity.Principal, p *models.Policy) (*models.Policy, error)
	...
	ReadPolicyVersions(ctx context.Context, id string) (oas.PolicyVersions, error)
	ReadPolicyVersion(ctx context.Context, id string, version int) (*oas.PolicyVersion, error)
	UpdatePolicy(ctx context.Context, user identity.Principal, prev *models.Policy, lastIndex uint64, p *models.Policy) (*models.Policy, error)
	DeletePolicy(ctx context.Context, user identity.Principal, prev *models.Policy, lastIndex uint64) (*models.Policy, error)
