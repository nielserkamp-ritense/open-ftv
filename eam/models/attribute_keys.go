package models

// ContextKey is a distinct type for values stored in a context.Context, to avoid collisions with other packages
// using the same underlying string as a different key.
type ContextKey string

// List of standard attribute keys.
const (
	AttrAPIKey                      = "api_key"
	AttrAction                      = "action"
	AttrBasicPswd                   = "basic_pswd"
	AttrBasicUser                   = "basic_user"
	AttrBody                        = "body"
	AttrClaims                      = "claims"
	AttrClientIP                    = "ip_address"
	AttrClientPrincipal             = "client_principal"
	AttrContentType                 = "content_type"
	AttrCoreUser                    = "core_user"
	AttrDeviceID                    = "device_id"
	AttrDoelbinding                 = "doelbinding"
	AttrEmail                       = "email"
	AttrFSC                         = "fsc"
	AttrFSCTransactionID ContextKey = "fsc_transaction_id"
	AttrGrondslag                   = "grondslag"
	AttrHTTP                        = "http"
	AttrHeaders                     = "headers"
	AttrHost                        = "host"
	AttrIssuer                      = "issuer"
	AttrJWT                         = "jwt"
	AttrMethod                      = "method"
	AttrPath                        = "path"
	AttrPathParts                   = "path_parts"
	AttrPreferredName               = "preferred_name"
	AttrPrincipal                   = "principal"
	AttrQuery                       = "query"
	AttrRequestTime                 = "request_time"
	AttrResource                    = "resource"
	AttrRoles                       = "roles"
	AttrRvvaID                      = "rvva_id"
	AttrScheme                      = "scheme"
	AttrParentSpanID     ContextKey = "parent_span_id"
	AttrSpanID           ContextKey = "span_id"
	AttrTaak                        = "taak"
	AttrTime                        = "time"
	AttrTraceID          ContextKey = "trace_id"
	AttrTraceParent                 = "traceparent" // https://www.w3.org/TR/trace-context/
	AttrValid                       = "valid"
	AttrZaakType                    = "zaak_type"
)
