...
func TestNew(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "all", opts: []Option{WithPEP(p2), WithLogger(log), WithPDP(p3), WithEntities(entities), WithContext(ctx)}, wantCtx: ctx, wantLog: log, wantPEP: p2, wantPDP: p3, wantEntities: entities},
	}
...
func TestAuthorize(t *testing.T) {
	...
	ep := pep.New(ctx, log)
	ip := pip2.New(ctx, log, pip2.WithFileStore("../../testdata/unittest/auth", true), pip2.WithFactories(cedar_embedded.NewAttributeBuilder(log), cedar_embedded.NewEntityBuilder(log)))
	ap := pap2.New(ctx, log, pap2.WithLanguage("cedar"), pap2.WithFileStore("../../testdata/unittest/auth/policies", true))
	dp := cedar_embedded.NewController(pdp.WithLogger(log), pdp.WithContext(ctx), pdp.WithPIP(ip), pdp.WithPAP(ap), pdp.WithPEP(ep))

	authenticator := authentication2.NewBCrypt(authentication2.WithContext(ctx), authentication2.WithLogger(log), authentication2.WithEntities(models.NewEntitySet(ip)))
