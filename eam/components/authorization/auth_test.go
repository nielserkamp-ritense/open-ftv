...
func TestAuthorize(t *testing.T) {
	...
	ep := pep.New(ctx, log)
	ip := pip.New(ctx, log, pip.WithFileStore("../../../testdata/unittest/auth", true), pip.WithFactories(cedar.NewAttributeBuilder(log), cedar.NewEntityBuilder(log)))
	ap := pap.New(ctx, log, pap.WithLanguage("cedar"), pap.WithFileStore("../../../testdata/unittest/auth/policies", true))
	dp := cedar.NewController(pdp.WithLogger(log), pdp.WithContext(ctx), pdp.WithPIP(ip), pdp.WithPAP(ap), pdp.WithPEP(ep))

	authenticator := authentication.NewBCrypt(authentication.WithContext(ctx), authentication.WithLogger(log), authentication.WithEntities(models.NewEntitySet(ip)))
